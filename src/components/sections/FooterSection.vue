<script setup>
import { RouterLink } from 'vue-router'
import { computed, onMounted } from 'vue'
import { useNavStore } from '@/stores/navStore'
import { useSettingsStore } from '@/stores/settingsStore'
import TrademarkTermsPolicy from '@/components/footer/TrademarkTermsPolicy.vue'
import bertonLogo from '@/assets/logos/berton_logo.png'

const navStore = useNavStore()
const settingsStore = useSettingsStore()

onMounted(() => {
  navStore.subscribeToNav()
})

const footerLinks = computed(() => navStore.footerItems)

const props = defineProps({
  data: {
    type: Object,
    default: () => ({
      brandName: '',
      description: '',
      columns: [
        { type: 'brand', title: '' },
        { type: 'nav', title: 'Navigation', group: '' },
        { type: 'contact', title: 'Get in Touch' },
      ],
    }),
  },
  styling: {
    type: Object,
    default: () => ({}),
  },
})

const socialIcons = {
  facebook:
    'M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.791-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z',
  instagram:
    'M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z',
  twitter:
    'M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z',
  linkedin:
    'M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z',
  youtube:
    'M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93 1.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-1.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z',
}

const getSocialIcon = (platform) => {
  if (!platform) return null
  const key = platform.toLowerCase()
  if (key.includes('facebook')) return socialIcons.facebook
  if (key.includes('instagram')) return socialIcons.instagram
  if (key.includes('twitter') || key.includes('x')) return socialIcons.twitter
  if (key.includes('linkedin')) return socialIcons.linkedin
  if (key.includes('youtube')) return socialIcons.youtube
  return null
}

const columns = computed(() => {
  if (props.data.columns && props.data.columns.length > 0)
    return props.data.columns
  return [
    { type: 'brand', title: '' },
    { type: 'nav', title: 'Navigation', group: '' },
    { type: 'contact', title: 'Get in Touch' },
  ]
})

const getLinksForGroup = (group) => {
  if (!group) {
    return footerLinks.value.filter((l) => !l.footerGroup)
  }
  return footerLinks.value.filter((l) => l.footerGroup === group)
}
</script>

<template>
  <footer
    :class="[
      styling.backgroundColor || 'bg-slate-900',
      styling.textColor || 'text-slate-300',
      styling.padding || 'py-16',
    ]"
  >
    <div class="container mx-auto px-4">
      <div
        class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-12 gap-y-10 md:gap-x-8"
      >
        <template v-for="(col, index) in columns" :key="index">
          <!-- Column 1: Brand -->
          <div v-if="col.type === 'brand'" class="md:col-span-2 xl:col-span-4">
            <h3
              v-if="settingsStore.settings.site_name"
              class="text-white font-medium mb-4"
            >
              {{ settingsStore.settings.site_name }}
            </h3>
            <p v-if="data.description" class="text-sm leading-relaxed">
              {{ data.description }}
            </p>
            <div class="mt-6 flex flex-col items-start gap-4">
              <p class="text-sm text-slate-400 mb-2 lg:mb-0">A subsidiary of</p>
              <a
                href="https://bertongoc.com/"
                target="_blank"
                rel="noopener noreferrer"
              >
                <div class="bg-white/95 p-2 rounded hover:bg-white">
                  <img :src="bertonLogo" alt="Berton Group" class="h-8" />
                </div>
              </a>
            </div>
            <!-- Social Icons -->
            <div
              v-if="
                settingsStore.settings.contact_info?.social_platforms?.length
              "
              class="flex gap-4 mt-6 flex-wrap"
            >
              <a
                v-for="(social, index) in settingsStore.settings.contact_info
                  .social_platforms"
                :key="index"
                :href="social.url"
                target="_blank"
                class="hover:text-white transition-colors"
                :title="social.platform"
              >
                <svg
                  v-if="getSocialIcon(social.platform)"
                  class="w-5 h-5"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path :d="getSocialIcon(social.platform)" />
                </svg>
                <span v-else class="material-symbols-outlined text-xl"
                  >public</span
                >
              </a>
            </div>
          </div>

          <!-- Column 2: Quick Links -->
          <div
            v-else-if="col.type === 'nav'"
            class="xl:col-start-7 xl:col-span-2"
          >
            <h4 class="text-white text-xs font-medium mb-6">{{ col.title }}</h4>
            <ul class="space-y-3 text-sm">
              <li
                v-for="link in getLinksForGroup(col.group)"
                :key="link.id || link.name"
              >
                <RouterLink
                  :to="link.to || link.url || '/'"
                  class="hover:text-white transition-colors"
                  >{{ link.name || link.label }}</RouterLink
                >
              </li>
            </ul>
          </div>

          <!-- Column 3: Contact -->
          <div
            v-else-if="col.type === 'contact'"
            class="xl:col-start-10 xl:col-span-3"
          >
            <h4 class="text-white text-xs font-medium mb-6">{{ col.title }}</h4>
            <ul class="space-y-4 text-sm">
              <template
                v-if="
                  settingsStore.settings.contact_info?.addresses?.filter(
                    (a) => a,
                  ).length
                "
              >
                <li
                  v-for="address in settingsStore.settings.contact_info.addresses.filter(
                    (a) => a,
                  )"
                  :key="address"
                  class="flex items-start gap-3"
                >
                  <span class="material-symbols-outlined text-lg shrink-0"
                    >location_on</span
                  >
                  <span>{{ address }}</span>
                </li>
              </template>

              <template
                v-if="
                  settingsStore.settings.contact_info?.postal_addresses?.filter(
                    (p) => p,
                  ).length
                "
              >
                <li
                  v-for="address in settingsStore.settings.contact_info.postal_addresses.filter(
                    (p) => p,
                  )"
                  :key="address"
                  class="flex items-center gap-3"
                >
                  <span class="material-symbols-outlined text-lg shrink-0"
                    >markunread_mailbox</span
                  >
                  <span>{{ address }}</span>
                </li>
              </template>

              <template
                v-if="
                  settingsStore.settings.contact_info?.emails?.filter((e) => e)
                    .length
                "
              >
                <li
                  v-for="email in settingsStore.settings.contact_info.emails.filter(
                    (e) => e,
                  )"
                  :key="email"
                  class="flex items-center gap-3"
                >
                  <span class="material-symbols-outlined text-lg shrink-0"
                    >mail</span
                  >
                  <a
                    :href="`mailto:${email}`"
                    class="hover:text-white transition-colors"
                    >{{ email }}</a
                  >
                </li>
              </template>

              <template
                v-if="
                  settingsStore.settings.contact_info?.phones?.filter((p) => p)
                    .length
                "
              >
                <li
                  v-for="phone in settingsStore.settings.contact_info.phones.filter(
                    (p) => p,
                  )"
                  :key="phone"
                  class="flex items-center gap-3"
                >
                  <span class="material-symbols-outlined text-lg shrink-0"
                    >call</span
                  >
                  <a
                    :href="`tel:${phone}`"
                    class="hover:text-white transition-colors"
                    >{{ phone }}</a
                  >
                </li>
              </template>
            </ul>
          </div>
        </template>
      </div>
      <!-- Copyright -->
      <TrademarkTermsPolicy class="mt-10" />
    </div>
  </footer>
</template>
